...
####################################################################################################
# def report_iou_from_files(stage_name, label_file, pred_file, ca_prob, n_prob, c_prob, out_path, R_sphere = 2.0):
def report_iou_from_files(stage_name, label_file, pred_file, R_sphere = 2.0):
    """
    ...
    gt_vol, gt_info = read_atom_label_file(label_file)
    gt_atoms = extract_atoms_from_label(gt_vol, gt_info, id_to_name=id_to_name, default_score=1.0)
    
    pred_vol,  pred_info = read_atom_label_file(pred_file)
...
# for CA
def report_iou_from_files_CA(stage_name, label_file, pred_file, R_sphere = 4.2):
    """
...
def build_parser():
    ...
    p.add_argument("--mrc_fin", help="v1 or v2 final MRC（1=CA,2=N,3=C）")
    p.add_argument("--R_distance",required=True, type=float, default=0.0, help="distance for IoU calculation")
    # p.add_argument("--output_dir", help="檔案輸出路徑")
    return p

def main1(): # print v1 result
    parser = build_parser()
    ...
    args.mrc_fin = f"/media/ray-suen/TRANSCEND1/huei/v1_v2_final/true_parms_cluster_v2/{args.emd_id}/lamda_40/final_atom_emd_normalized_map.mrc"

    # if args.output_dir is None:
    #     args.output_dir  = f" /media/ray-suen/TRANSCEND1/huei/v1_v2_final/v1/{args.emd_id}/output" 
    ...
    # af_cm, af_am = report_iou_from_files("Final", args.label_mrc, args.mrc_fin, radius=args.R_distance)
    cm, per_class, macro_f1, macro_iou = report_iou_from_files("Final", args.label_mrc,  args.mrc_fin, R_sphere = args.R_distance)
    trial_record = {
        "R_distance":  args.R_distance,
        "CA_atom_iou_metrics" : per_class["CA"],
        "N_atom_iou_metrics" : per_class["N"],
        "C_atom_iou_metrics" : per_class["C"],
        "confusion_matrix" : cm ,
        "Final_f1" : np.round(macro_f1),
        "Final_IoU": np.round(macro_iou)
    }
    return trial_record    

def main2(): # print v2 result
    parser = build_parser()
    args = parser.parse_args()
    ...
    args.mrc_fin = f"/media/ray-suen/TRANSCEND1/huei/v1_v2_final/change_c_true_parms_cluster_v2/{args.emd_id}/final_atom_emd_normalized_map.mrc"
    
    # if args.output_dir is None:
    #     args.output_dir  = f" /media/ray-suen/TRANSCEND1/huei/v1_v2_final/v2/{args.emd_id}/output" 
...
def main_MICA(): # print v2 result
    parser = build_parser()
    args = parser.parse_args()

    # MICA (評估原始流程上的聚類效果)
    # mrc_thr = f"/media/ray-suen/TRANSCEND1/huei/v1_v2_final/MICA_optuna/{args.emd_id}/MICA_after_clustering_train/MICA_CA_emd_normalized_map.mrc"
    # ca_pts_txt = f"/media/ray-suen/TRANSCEND1/huei/MICA/output/results/{args.emd_id}/final_output/{args.emd_id}_CA_after_clustering.txt"

    # Cryo-Atom (評估原始流程上的聚類效果)
    # /media/ray-suen/TRANSCEND1/huei/v1_v2_final/Cryo_Atom_out/0689/Cryo_Atom_after_clustering_train
    # mrc_thr = f"/media/ray-suen/TRANSCEND1/huei/v1_v2_final/Cryo_Atom_out/{args.emd_id}/Cryo_Atom_after_clustering_train/Cryo_Atom_CA_emd_normalized_map.mrc"
    # ca_pts_txt = f"/media/ray-suen/TRANSCEND1/huei/CryoAtom/out/{args.emd_id}/see_alpha_output/CA_after_clustering.txt"
    
    import ast
    import os
    # ca_pts = []
    # if os.path.exists(ca_pts_txt):
    #     with open(ca_pts_txt, 'r') as f:
    #         for line in f:
    #             line = line.strip()
    #             if not line: continue
    #             try:
    #                 coords, prob = ast.literal_eval(line)
    #                 ca_pts.append((float(coords[0]), float(coords[1]), float(coords[2]), float(prob)))
        ...
        with open(ca_pts_txt, 'r') as f:
            for line in f:
                ...
                try:
                        ...
                        ca_pts.append((float(coords[0]), float(coords[1]), float(coords[2])))
                except Exception as e:
                    pass
    ...
    cm, per_class, macro_f1, macro_iou = report_iou_from_files_CA("Final", args.label_mrc,  args.mrc_fin, R_sphere = args.R_distance)
    trial_record = {
        "R_distance":  args.R_distance,
        "CA_atom_iou_metrics" : per_class["CA"],
        # "N_atom_iou_metrics" : per_class["N"],
...
def main_cryo(): # print v2 result
    parser = build_parser()
    args = parser.parse_args()

    # Cryo2Struct (評估原始流程上的聚類效果)
    mrc_thr = f"/media/ray-suen/TRANSCEND1/huei/v1_v2_final/Cryo2struct_optuna_ca/{args.emd_id}/Cryo2struct_after_clustering_train/Cryo2Struct_CA_emd_normalized_map.mrc"
    ca_pts_txt = f"/media/ray-suen/TRANSCEND1/huei/v1_v2_final/Cryo2struct_optuna_ca/{args.emd_id}/Cryo2struct_after_clustering_train/{args.emd_id}_cluster_transition_ca.txt"
    

    # Cryo2Struct2 (評估原始流程上的聚類效果)
    # mrc_thr = f"/media/ray-suen/TRANSCEND1/huei/v1_v2_final/Cryo2struct2_optuna_ca/{args.emd_id}/Cryo2struct2_after_clustering_train/Cryo2Struct_CA_emd_normalized_map.mrc"
    # ca_pts_txt = f"/media/ray-suen/TRANSCEND1/huei/v1_v2_final/Cryo2struct2_optuna_ca/{args.emd_id}/Cryo2struct2_after_clustering_train/{args.emd_id}_cluster_transition_ca.txt"
    ...
    if os.path.exists(ca_pts_txt):
        with open(ca_pts_txt, 'r') as f:
            for line in f:
                ...
                if not line: continue
                try:
                    # 嘗試以 ast.literal_eval 解析 (例如針對 `(x, y, z)` 的格式)
                    try:
                        coords = ast.literal_eval(line)
                        ca_pts.append((float(coords[0]), float(coords[1]), float(coords[2])))
                        continue
                    ...
                    # 嘗試以空格分隔解析 (例如針對 `39.8 56.1 70.5` 的格式)
                    parts = line.split()
                    if len(parts) >= 3:
                        ca_pts.append((float(parts[0]), float(parts[1]), float(parts[2])))
                except Exception as e:
                    pass
...
def main_cryo_CA(): # print v2 result
    parser = build_parser()
    args = parser.parse_args()
    ...
    args.mrc_fin = f"/media/ray-suen/TRANSCEND1/huei/org_code/Cryo2Struct-main/input/lamdba_10/{args.emd_id}/final_atom_emd_normalized_map.mrc"
